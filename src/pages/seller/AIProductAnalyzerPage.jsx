...
import { Scan, Upload, Camera, Trash2, Sparkles, CheckCircle2, AlertCircle, Star, RefreshCw, Edit3, Send, UserX, AlertTriangle, ShieldAlert, FileText } from 'lucide-react';
import confetti from 'canvas-confetti';
import { useApp } from '../../context/AppContext';
import { analyzeProductImage, SCAN_STEPS } from '../../services/aiService';
import { CameraCaptureModal } from '../../components/common/CameraCaptureModal';
const SpecField = ({ label, value, badge, className = '' }) => (<div className={`p-3.5 rounded-2xl bg-heritage-ivory/60 border border-heritage-sand/80 flex flex-col justify-between ${className}`}>
    <div className="flex items-center justify-between mb-1">
      <span className="text-[11px] font-bold text-heritage-charcoal/60 uppercase tracking-wider block">
        {label}
      </span>
      {badge && (<span className="text-[9px] font-semibold px-2 py-0.5 rounded-full bg-emerald-100 text-emerald-800 border border-emerald-200">
          {badge}
        </span>)}
    </div>
...
const ConfidenceGauge = ({ confidence }) => {
    ...
    return (<div className="flex items-center space-x-3.5 p-3.5 rounded-2xl bg-gradient-to-r from-heritage-gold/15 to-heritage-sand/30 border border-heritage-gold/40">
      <div className="relative w-14 h-14 shrink-0 flex items-center justify-center">
        <svg className="w-14 h-14 -rotate-90 transform" viewBox="0 0 60 60" aria-hidden="true">
          <circle cx="30" cy="30" r={radius} className="text-heritage-sand/80" strokeWidth="5" stroke="currentColor" fill="transparent"/>
          <circle cx="30" cy="30" r={radius} className="text-emerald-600 transition-all duration-1000 ease-out" strokeWidth="5" strokeDasharray={circumference} strokeDashoffset={strokeDashoffset} strokeLinecap="round" stroke="currentColor" fill="transparent"/>
        </svg>
...
const getCraftingTechnique = (result) => {
    if (result.craftingTechnique)
        return result.craftingTechnique;
    const c = (result.category || '').toLowerCase();
    const m = (result.material || '').toLowerCase();
    if (c.includes('pottery') || m.includes('clay'))
        return 'Wheel-Thrown & Hand-Etched Terracotta';
    if (c.includes('wood') || m.includes('wood'))
        return 'Hand-Chiseled Relief & Lacquer Finishing';
    if (c.includes('metal') || m.includes('brass'))
        return 'Traditional Dhokra Lost-Wax Casting';
    if (c.includes('textile') || m.includes('silk') || m.includes('cotton'))
        return 'Authentic Pitloom / Handloom Weaving';
    if (c.includes('bamboo') || c.includes('cane'))
        return 'Fine Split-Reed Hand-Weaving';
    if (c.includes('jute'))
        return 'Twisted Natural Jute Fiber Braiding';
    if (c.includes('painting') || c.includes('art'))
        return 'Natural Mineral Pigment Freehand Brushwork';
...
export const AIProductAnalyzerPage = () => {
    ...
    return (<div className="min-h-screen bg-heritage-ivory py-8 px-4 sm:px-6 lg:px-8">
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <div className="bg-gradient-to-r from-heritage-brown via-heritage-brown-dark to-heritage-terracotta text-white rounded-3xl p-6 sm:p-8 shadow-3d-lg border-2 border-heritage-gold/40 flex flex-col md:flex-row md:items-center justify-between gap-5">
          <div className="flex items-start sm:items-center space-x-4">
            <img src="/karigasetu-logo.png" alt="Karigasetu.ai Logo" className="w-16 h-16 sm:w-20 sm:h-20 rounded-2xl object-contain bg-white/10 p-1 border-2 border-heritage-gold/60 shadow-md shrink-0"/>
            <div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          ...
          <div className="lg:col-span-6 bg-white rounded-3xl p-6 border border-heritage-terracotta/20 shadow-3d space-y-4">
            <div className="flex items-center justify-between">
              ...
              {selectedImage && (<button type="button" onClick={handleRemoveImage} className="text-xs text-rose-600 hover:text-rose-800 font-bold flex items-center space-x-1 transition cursor-pointer">
            ...
            <div onDragOver={(e) => e.preventDefault()} onDrop={handleDrop} onClick={() => {
            if (!selectedImage && !isAnalyzing) {
                fileInputRef.current?.click();
            }
        }} className={`relative aspect-square w-full rounded-2xl overflow-hidden border-2 border-dashed border-heritage-sand bg-heritage-ivory/50 flex flex-col items-center justify-center transition shadow-inner group ${!selectedImage && !isAnalyzing
            ? 'cursor-pointer hover:border-heritage-terracotta hover:bg-heritage-sand/30'
            : ''}`}>
              {selectedImage ? (<>
                  <img src={selectedImage} alt="Handicraft Upload Preview" className={`w-full h-full object-contain bg-heritage-ivory/80 transition duration-500 ${isAnalyzing ? 'brightness-90 contrast-110' : ''}`}/>

                  {/* Change Photo Overlay on hover */}
                  {!isAnalyzing && (<div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center space-x-3">
            ...
            <div className="grid grid-cols-2 gap-3 pt-1">
              <button type="button" onClick={() => fileInputRef.current?.click()} className="py-3 px-3 rounded-2xl border border-heritage-sand bg-heritage-ivory/60 hover:bg-heritage-sand text-xs font-bold text-heritage-brown flex items-center justify-center space-x-2 transition cursor-pointer">
                <Upload className="w-4 h-4 text-heritage-terracotta" aria-hidden="true"/>
                <span>{selectedImage ? 'Replace Image' : 'Browse File'}</span>
              </button>

              <button type="button" onClick={() => setCameraActive(true)} className="py-3 px-3 rounded-2xl border border-heritage-sand bg-heritage-ivory/60 hover:bg-heritage-sand text-xs font-bold text-heritage-brown flex items-center justify-center space-x-2 transition cursor-pointer">
                <Camera className="w-4 h-4 text-heritage-brown" aria-hidden="true"/>
                <span>{selectedImage ? 'Retake Photo' : 'Take Photo'}</span>
              </button>
            </div>

            {uploadError && (<div className="p-3 bg-rose-50 border border-rose-200 rounded-2xl text-rose-800 text-xs font-semibold flex items-center space-x-2">
                <AlertCircle className="w-4 h-4 text-rose-600 shrink-0" aria-hidden="true"/>
                <span>{uploadError}</span>
              </div>)}

            {/* MANDATORY PROMINENT BUTTON: [ Analyze with AI ] */}
            <button type="button" disabled={!selectedImage || isAnalyzing} onClick={handleStartAnalysis} className={`w-full py-4 rounded-2xl font-black text-sm sm:text-base shadow-3d-lg transition-all flex items-center justify-center space-x-2 uppercase tracking-wider cursor-pointer ${!selectedImage || isAnalyzing
            ...
            {(isAnalyzing || analysisResult) && (<div className="pt-4 border-t border-heritage-sand space-y-2">
                ...
                return (<div key={step.id} className={`flex items-center justify-between p-2.5 rounded-xl text-xs transition ${isDone
                        ...
                        <div className="flex items-center space-x-2.5 min-w-0">
                          {isDone ? (<CheckCircle2 className="w-4 h-4 text-emerald-600 shrink-0" aria-hidden="true"/>) : (<div className="w-4 h-4 rounded-full border border-current flex items-center justify-center text-[10px] shrink-0 font-bold">
                              {step.id}
                            </div>)}
          ...
          <div className="lg:col-span-6 space-y-6">
            {!analysisResult && !isAnalyzing ? (<div className="bg-white rounded-3xl p-8 sm:p-12 border border-heritage-sand shadow-3d text-center space-y-4">
              ...
              </div>) : isAnalyzing ? (<div className="bg-white rounded-3xl p-8 sm:p-12 border border-heritage-sand shadow-3d text-center space-y-4">
              ...
              </div>) : analysisResult ? (analysisResult.isValidCraft === false || analysisResult.isHumanSubject === true || analysisResult.isDocumentSubject === true ? (
        ...
        <div className="bg-white rounded-3xl p-6 sm:p-8 border-2 border-rose-300 shadow-3d-lg space-y-6 animate-fade-in">
                  <div className="p-4 bg-rose-50 border border-rose-200 rounded-2xl text-rose-950 flex items-start space-x-3">
                    <div className="w-10 h-10 rounded-xl bg-rose-200 text-rose-800 flex items-center justify-center shrink-0">
                      {analysisResult.isDocumentSubject ? (<FileText className="w-5 h-5 text-rose-700" aria-hidden="true"/>) : analysisResult.isHumanSubject ? (<UserX className="w-5 h-5 text-rose-700" aria-hidden="true"/>) : (<ShieldAlert className="w-5 h-5 text-rose-700" aria-hidden="true"/>)}
                    </div>
                    <div>
                ...
                : analysisResult.isHumanSubject
                  ...
                  <div className="space-y-2 pt-2">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      <button type="button" onClick={() => fileInputRef.current?.click()} className="py-3 px-4 rounded-xl bg-heritage-terracotta hover:bg-heritage-terracotta-dark text-white font-bold text-xs shadow-md transition flex items-center justify-center space-x-2 cursor-pointer">
                        <Upload className="w-4 h-4" aria-hidden="true"/>
                        <span>Upload Craft Image</span>
                      </button>
                      <button type="button" onClick={() => setCameraActive(true)} className="py-3 px-4 rounded-xl border border-heritage-sand bg-heritage-sand/40 hover:bg-heritage-sand text-heritage-brown font-bold text-xs transition flex items-center justify-center space-x-2 cursor-pointer">
                        <Camera className="w-4 h-4" aria-hidden="true"/>
                        <span>Take Photo</span>
                      </button>
                    </div>
        ...
        <div className="bg-white rounded-3xl p-6 sm:p-8 border-2 border-heritage-gold shadow-3d-lg space-y-6 relative overflow-hidden animate-fade-in">
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-heritage-sand/80 pb-4">
                    <div className="flex items-center space-x-3">
                      <img src="/karigasetu-logo.png" alt="Karigasetu" className="w-10 h-10 rounded-full object-contain border border-heritage-gold/50 shadow-xs"/>
                      <div>
                    ...
                    <div className="flex flex-col sm:items-end gap-1">
                      <span className={`text-[11px] font-black uppercase tracking-wider px-3 py-1 rounded-full border self-start sm:self-auto ${analysisResult.isLiveAi || analysisResult.analysisSource === 'live_ai'
                ...
                : 'bg-amber-100 text-amber-900 border-amber-300'}`}>
                  ...
                  {publishedSuccess && (<div className="p-4 bg-emerald-100 border border-emerald-300 rounded-2xl text-emerald-900 text-xs font-bold flex items-center space-x-2 animate-bounce">
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      <SpecField label="Product Name" value={analysisResult.productName || 'Handcrafted Heritage Item'} className="sm:col-span-2"/>
                      <SpecField label="Craft Category" value={analysisResult.category || 'Handicraft'}/>
                      <SpecField label="Craft Type" value={analysisResult.craftType || analysisResult.model || 'Traditional Indian Craft'}/>
                      <SpecField label="Material" value={analysisResult.material || 'Natural Materials'}/>
                      <SpecField label="Craft Technique" value={craftTechnique}/>
                    </div>
                  ...
                  <div className="space-y-4 pt-2">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      <SpecField label="Primary Color" value={analysisResult.primaryColor || 'Natural Earth Tone'}/>
                      <SpecField label="Secondary Color" value={analysisResult.secondaryColor || 'Natural Accent'}/>
                      <SpecField label="Shape" value={analysisResult.shape || 'Sculptural Artisan Form'}/>
                      <SpecField label="Texture" value={analysisResult.texture || 'Hand-worked natural grain texture'}/>
                      <SpecField label="Dimensions" badge="AI ESTIMATED" value={dimensionsStr}/>
                      <SpecField label="Craft Finish" value={analysisResult.craftFinish || 'Organic hand-buffed protective finish'}/>
                    </div>
                  </div>
                  ...
                  <div className="space-y-4 pt-2">
                    ...
                    {analysisResult.culturalSignificance && (<div className="p-3.5 bg-heritage-sand/20 rounded-2xl border border-heritage-sand/60">
                  ...
                  <div className="pt-4 border-t border-heritage-sand flex flex-col sm:flex-row items-center gap-3">
                    <button type="button" onClick={handlePublishDirectly} className="w-full sm:flex-1 py-3.5 px-5 rounded-2xl bg-gradient-to-r from-heritage-terracotta to-amber-600 hover:from-heritage-terracotta-dark hover:to-amber-700 text-white font-black text-xs sm:text-sm shadow-md hover:shadow-lg transition flex items-center justify-center space-x-2 cursor-pointer">
                      <Send className="w-4 h-4" aria-hidden="true"/>
                      <span>Publish Product</span>
                    </button>

                    <button type="button" onClick={handleEditBeforePublish} className="w-full sm:w-auto px-5 py-3.5 rounded-2xl bg-heritage-sand hover:bg-heritage-sand/80 text-heritage-brown font-bold text-xs transition flex items-center justify-center space-x-1.5 cursor-pointer">
                      <Edit3 className="w-4 h-4" aria-hidden="true"/>
                      <span>Edit Details</span>
                    </button>

                    <button type="button" onClick={handleStartAnalysis} className="w-full sm:w-auto px-5 py-3.5 rounded-2xl border-2 border-heritage-sand hover:bg-white text-heritage-charcoal font-bold text-xs transition flex items-center justify-center space-x-1.5 cursor-pointer">
      ...
      <CameraCaptureModal isOpen={cameraActive} onClose={() => setCameraActive(false)} onCapture={(dataUrl) => {
            setFileObject(null);
