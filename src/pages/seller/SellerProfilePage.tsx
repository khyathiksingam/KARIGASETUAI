...
export const SellerProfilePage: React.FC = () => {
  ...
  const voucherFlyerInputRef = useRef<HTMLInputElement | null>(null);

  // Active section tab
  const [activeTab, setActiveTab] = useState<'profile' | 'banking' | 'coupons'>('profile');
  ...
  const [passbookImage, setPassbookImage] = useState<string>(
    initialBank?.passbook_image || 'https://images.unsplash.com/photo-1554224155-8d04cb21cd6c?w=600&auto=format&fit=crop&q=80'
  );
  ...
  const [voucherFlyerImage, setVoucherFlyerImage] = useState<string | null>(null);
  const [couponSavedMsg, setCouponSavedMsg] = useState<string | null>(null);
  ...
  const handlePassbookUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files[0]) {
      ...
      reader.onload = (event) => {
        if (event.target?.result) {
          const newDoc = event.target.result as string;
  ...
  const handleVoucherFlyerUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files[0]) {
      const file = e.target.files[0];
      const reader = new FileReader();
      reader.onload = (event) => {
        if (event.target?.result) {
          setVoucherFlyerImage(event.target.result as string);
        }
      };
      reader.readAsDataURL(file);
    }
    ...
    <div className="min-h-screen bg-heritage-ivory py-8 px-4 sm:px-6 lg:px-8">
      <div className="max-w-4xl mx-auto space-y-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          ...
          <Link
            to={`/artisan/${currentUser?.username || 'ravicrafts'}`}
            className="px-4 py-2.5 rounded-xl bg-heritage-brown text-heritage-gold-light hover:bg-heritage-brown-dark font-bold text-xs flex items-center space-x-2 transition shadow-sm"
          >
        ...
        <div className="bg-white rounded-3xl p-6 sm:p-8 border border-heritage-terracotta/20 shadow-3d flex flex-col sm:flex-row items-center space-y-4 sm:space-y-0 sm:space-x-6">
          ...
          <div className="text-center sm:text-left space-y-1 flex-1">
            ...
            <button
              ...
              className="text-[11px] font-bold text-heritage-terracotta hover:underline inline-flex items-center mt-1"
            >
        ...
        {/* TAB 1: PROFILE & STORY */}
        {activeTab === 'profile' && (
          <div className="bg-white rounded-3xl p-6 sm:p-8 border border-heritage-sand shadow-3d animate-fade-in">
            <h3 className="font-serif font-bold text-lg text-heritage-brown mb-4 pb-3 border-b border-heritage-sand">
              Update Profile Information
            </h3>

            {saved && (
              <div className="mb-4 p-3.5 rounded-xl bg-emerald-50 border border-emerald-200 text-emerald-800 text-xs font-bold flex items-center space-x-2">
            ...
            <form onSubmit={handleSaveProfile} className="space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <LocationInput
                    ...
                    placeholder="e.g. Kothagudem or Warangal"
                  />
                </div>
              ...
              <button
                type="submit"
                className="py-3 px-6 rounded-xl bg-heritage-terracotta hover:bg-heritage-terracotta-dark text-white font-bold text-xs shadow-md transition"
              >
        ...
        {activeTab === 'banking' && (
          <div className="bg-white rounded-3xl p-6 sm:p-8 border border-heritage-sand shadow-3d space-y-6 animate-fade-in">
            ...
            <form onSubmit={handleSaveBankDetails} className="space-y-5">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-4 py-2.5 rounded-xl border border-heritage-sand text-xs font-medium focus:border-heritage-terracotta outline-none bg-white"
                  />
                </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <div className="flex items-center justify-between mb-1">
                    ...
                    <button
                      ...
                      className="text-[10px] text-heritage-terracotta font-bold hover:underline flex items-center space-x-1"
                    >
                  ...
                  <input
                    ...
                    className="w-full px-4 py-2.5 rounded-xl border border-heritage-sand text-xs font-mono font-bold focus:border-heritage-terracotta outline-none bg-white"
                  />
                </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-4 py-2.5 rounded-xl border border-heritage-sand text-xs font-mono uppercase font-bold focus:border-heritage-terracotta outline-none bg-white"
                  />
                </div>
              </div>
              ...
              <div>
                ...
                <div className="relative">
                  <input
                    ...
                    placeholder="e.g. ravikumar.artisan@sbi or 9848012345@paytm"
                    className="w-full px-4 py-2.5 rounded-xl border border-heritage-sand text-xs font-mono font-bold focus:border-heritage-terracotta outline-none bg-white pr-24"
                  />
                ...
                <div className="grid grid-cols-1 sm:grid-cols-12 gap-4 items-center">
                      ...
                      <>
                        ...
                        <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 flex items-center justify-center transition">
                          <button
                            ...
                            className="px-3 py-1.5 rounded-xl bg-white text-heritage-brown text-xs font-bold shadow"
                          >
                            Change Document
                          </button>
                        </div>
                  ...
                  <div className="sm:col-span-7 space-y-3">
                    <button
                      type="button"
                      onClick={() => passbookInputRef.current?.click()}
                      className="w-full py-3 px-4 rounded-xl border border-heritage-terracotta bg-heritage-sand/30 hover:bg-heritage-sand text-xs font-bold text-heritage-terracotta flex items-center justify-center space-x-2 transition"
                    >
                      <Upload className="w-4 h-4" />
              ...
              <button
                type="submit"
                className="py-3 px-6 rounded-xl bg-heritage-terracotta hover:bg-heritage-terracotta-dark text-white font-bold text-xs shadow-md transition flex items-center space-x-2"
              >
              ...
              <form onSubmit={handleCreateCoupon} className="space-y-4">
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  <div>
                    ...
                    <input
                      ...
                      placeholder="e.g. CRAFT2026 or DIWALI15"
                      className="w-full px-4 py-2.5 rounded-xl border border-heritage-sand text-xs font-mono uppercase font-bold focus:border-heritage-terracotta outline-none"
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <select
                      ...
                      className="w-full px-4 py-2.5 rounded-xl border border-heritage-sand text-xs font-bold focus:border-heritage-terracotta outline-none bg-white"
                    >
                  ...
                  <div>
                    ...
                    <input
                      ...
                      required
                      min={1}
                      max={discountType === 'percentage' ? 80 : 5000}
                      value={discountValue}
                      onChange={(e) => setDiscountValue(Number(e.target.value))}
                      className="w-full px-4 py-2.5 rounded-xl border border-heritage-sand text-xs font-bold focus:border-heritage-terracotta outline-none"
                    />
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      placeholder="e.g. Special festive discount on all handloom orders"
                      className="w-full px-4 py-2.5 rounded-xl border border-heritage-sand text-xs font-medium focus:border-heritage-terracotta outline-none"
                    />
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="flex items-center space-x-3">
                    <button
                      type="button"
                      onClick={() => voucherFlyerInputRef.current?.click()}
                      className="py-2.5 px-4 rounded-xl border border-dashed border-heritage-terracotta bg-heritage-sand/30 hover:bg-heritage-sand text-xs font-bold text-heritage-terracotta flex items-center space-x-1.5 transition"
                    >
                    ...
                    {voucherFlyerImage && (
                      <div className="flex items-center space-x-2">
                        <img
                          src={voucherFlyerImage}
                          alt="Voucher Flyer"
                          className="w-10 h-10 rounded-lg object-cover border border-heritage-sand"
                        />
                        <button
                          type="button"
                          onClick={() => setVoucherFlyerImage(null)}
                    ...
                    <input
                      type="file"
                      ref={voucherFlyerInputRef}
                      accept="image/*"
                      className="hidden"
                      onChange={handleVoucherFlyerUpload}
                    />
                  </div>
                </div>
            ...
            <div className="bg-white rounded-3xl p-6 sm:p-8 border border-heritage-sand shadow-3d space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {coupons.map((c) => (
                  <div
                    key={c.id}
                    className="p-4 rounded-2xl border border-dashed border-heritage-terracotta/40 bg-heritage-ivory/50 flex items-start justify-between space-x-3 relative overflow-hidden group hover:border-heritage-terracotta transition"
                  >
                    <div className="space-y-1">
                      <div className="flex items-center space-x-2">
                    ...
                    <button
                      ...
                      className="p-2 rounded-xl bg-white border border-heritage-sand hover:border-heritage-terracotta text-heritage-brown transition shrink-0"
