...
export const SellerCreditsPage: React.FC = () => {
  const { currentUser, creditTransactions } = useApp();

  const userTxs = creditTransactions.filter(
    (tx) => tx.seller_id === currentUser?.id || currentUser?.role === 'seller'
  );
                    ...
                    >
                      <div className="space-y-1">
                        <div className="flex items-center space-x-2">
                          ...
                          {isCurrent && (
                            <span className="bg-heritage-terracotta text-white text-[9px] font-bold px-2 py-0.5 rounded-full uppercase">
                              Active Tier
                            </span>
                          )}
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left text-xs">
              ...
              <tbody className="divide-y divide-heritage-sand/60">
                {userTxs.map((tx) => (
                  <tr key={tx.id} className="hover:bg-heritage-ivory/50">
