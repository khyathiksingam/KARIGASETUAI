...
import { Lock, User, Phone, CheckCircle, Shield, ArrowRight, AlertCircle, Mail, Zap, RefreshCw } from 'lucide-react';
...
export const LoginPage = () => {
    ...
    const [password, setPassword] = useState(() => {
        ...
        if (rem === 'seller_demo' || rem === 'ravi@ravicrafts.com')
            return 'Seller@123';
        if (rem === 'buyer_demo' || rem === 'ananya.s@heritagearts.in')
            return 'Buyer@123';
        return '';
    });
    const [otpTargetType, setOtpTargetType] = useState('mobile');
      ...
      <div className="max-w-md mx-auto w-full">
        ...
        <div className="bg-white rounded-3xl p-6 sm:p-8 border border-heritage-terracotta/20 shadow-3d">
          ...
          <div className="flex border-b border-heritage-sand pb-4 mb-6">
            ...
            setErrorMsg('');
        }} className={`flex-1 pb-2 text-xs font-bold text-center border-b-2 transition ${authMethod === 'credentials'
            ...
            setAuthMethod('otp');
            setErrorMsg('');
        }} className={`flex-1 pb-2 text-xs font-bold text-center border-b-2 transition ${authMethod === 'otp'
            ? 'border-heritage-terracotta text-heritage-terracotta'
            : 'border-transparent text-heritage-charcoal/60 hover:text-heritage-brown'}`}>
          ...
          {errorMsg && (<div className="mb-4 p-3 rounded-xl bg-red-50 border border-red-200 text-red-700 text-xs flex items-center space-x-2">
        ...
        <form onSubmit={handlePasswordLogin} className="space-y-4">
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input type="text" name="username" autoComplete="username" required value={identity} onChange={(e) => setIdentity(e.target.value)} placeholder="e.g. seller_demo or ravi@ravicrafts.com" className="w-full pl-10 pr-4 py-2.5 rounded-xl border border-heritage-sand bg-heritage-ivory/40 text-xs font-medium text-heritage-charcoal focus:border-heritage-terracotta focus:ring-1 focus:ring-heritage-terracotta outline-none"/>
                </div>
              </div>
              ...
              <div>
                <div className="flex items-center justify-between mb-1">
                  ...
                  <button type="button" onClick={() => alert('Demo Passwords: Seller@123 or Buyer@123')} className="text-[11px] text-heritage-terracotta font-semibold hover:underline">
                    Forgot Password?
                  </button>
                </div>
                <div className="relative">
                  <Lock className="w-4 h-4 text-heritage-charcoal/40 absolute left-3.5 top-3"/>
                  <input type="password" name="password" autoComplete="current-password" required value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Enter password (e.g. Seller@123)" className="w-full pl-10 pr-4 py-2.5 rounded-xl border border-heritage-sand bg-heritage-ivory/40 text-xs font-medium text-heritage-charcoal focus:border-heritage-terracotta focus:ring-1 focus:ring-heritage-terracotta outline-none"/>
                </div>
              </div>
              ...
              <div className="flex items-center justify-between pt-1">
                <label className="flex items-center space-x-2 cursor-pointer select-none">
                  <input type="checkbox" checked={rememberMe} onChange={(e) => setRememberMe(e.target.checked)} className="w-4 h-4 rounded text-heritage-terracotta border-heritage-sand focus:ring-heritage-terracotta cursor-pointer accent-heritage-terracotta"/>
              ...
              <button type="submit" className="w-full mt-2 py-3 rounded-xl bg-heritage-terracotta hover:bg-heritage-terracotta-dark text-white font-bold text-xs shadow-md transition">
                Sign In
              </button>
            </form>) : (
        ...
        <div className="space-y-4">
              {!otpSent ? (<form onSubmit={handleSendOtp} className="space-y-4">
                  ...
                  <div className="flex items-center space-x-2 bg-heritage-sand/40 p-1 rounded-xl">
                    <button type="button" onClick={() => setOtpTargetType('mobile')} className={`flex-1 py-1.5 px-3 rounded-lg text-xs font-bold transition flex items-center justify-center space-x-1.5 ${otpTargetType === 'mobile'
                    ? 'bg-heritage-terracotta text-white shadow-xs'
                    : 'text-heritage-brown hover:bg-white/50'}`}>
                      <Phone className="w-3.5 h-3.5"/>
                      <span>Mobile SMS / WhatsApp</span>
                    </button>
                    <button type="button" onClick={() => setOtpTargetType('email')} className={`flex-1 py-1.5 px-3 rounded-lg text-xs font-bold transition flex items-center justify-center space-x-1.5 ${otpTargetType === 'email'
                    ? 'bg-heritage-terracotta text-white shadow-xs'
                    : 'text-heritage-brown hover:bg-white/50'}`}>
                  ...
                  {otpTargetType === 'mobile' ? (<div>
                      <label className="block text-xs font-bold text-heritage-brown mb-1">
                        Mobile Number
                      </label>
                      <div className="relative">
                        <Phone className="w-4 h-4 text-heritage-charcoal/40 absolute left-3.5 top-3"/>
                        <input type="tel" required value={mobile} onChange={(e) => setMobile(e.target.value)} placeholder="+91 92814 32397" className="w-full pl-10 pr-4 py-2.5 rounded-xl border border-heritage-sand bg-heritage-ivory/40 text-xs font-medium text-heritage-charcoal focus:border-heritage-terracotta focus:ring-1 focus:ring-heritage-terracotta outline-none"/>
                      </div>
                      ...
                      <div className="relative">
                        ...
                        <input type="email" required value={email} onChange={(e) => setEmail(e.target.value)} placeholder="artisan@karigarsetu.ai" className="w-full pl-10 pr-4 py-2.5 rounded-xl border border-heritage-sand bg-heritage-ivory/40 text-xs font-medium text-heritage-charcoal focus:border-heritage-terracotta focus:ring-1 focus:ring-heritage-terracotta outline-none"/>
                      </div>
                      <p className="text-[11px] text-heritage-charcoal/60 mt-1">
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      <button type="button" onClick={() => setSelectedRole('seller')} className={`py-2 text-xs font-bold rounded-lg border transition ${selectedRole === 'seller'
                    ...
                    : 'border-heritage-sand text-heritage-brown bg-heritage-ivory/50'}`}>
                      ...
                      <button type="button" onClick={() => setSelectedRole('buyer')} className={`py-2 text-xs font-bold rounded-lg border transition ${selectedRole === 'buyer'
                  ...
                  <button type="submit" className="w-full py-3 rounded-xl bg-heritage-terracotta hover:bg-heritage-terracotta-dark text-white font-bold text-xs shadow-md transition flex items-center justify-center space-x-2">
                  ...
                  <div>
                    <div className="flex items-center justify-between mb-1">
                      ...
                      {generatedOtp && (<button type="button" onClick={() => setOtp(generatedOtp)} className="text-[11px] text-heritage-terracotta font-bold hover:underline flex items-center space-x-1">
                          <Zap className="w-3 h-3 text-amber-500"/>
                          <span>Auto-Fill {generatedOtp}</span>
                        </button>)}
                    </div>
                    <input type="text" maxLength={6} required value={otp} onChange={(e) => setOtp(e.target.value)} placeholder={generatedOtp || '123456'} className="w-full text-center tracking-widest text-xl font-mono font-black py-2.5 rounded-xl border-2 border-heritage-sand bg-white text-heritage-brown focus:border-heritage-terracotta outline-none shadow-inner"/>
                  </div>

                  <button type="submit" className="w-full py-3 rounded-xl bg-heritage-terracotta hover:bg-heritage-terracotta-dark text-white font-bold text-xs shadow-md transition">
                    Verify & Sign In
                  </button>

                  <div className="flex items-center justify-between text-xs pt-1">
                    <button type="button" disabled={resendCooldown > 0} onClick={() => handleSendOtp()} className={`font-semibold flex items-center space-x-1 ${resendCooldown > 0
          ...
          {/* Google Auth Button */}
          <button type="button" onClick={handleGoogleLogin} className="w-full py-2.5 px-4 rounded-xl border border-heritage-sand hover:bg-heritage-sand/40 text-xs font-bold text-heritage-brown flex items-center justify-center space-x-2 transition">
