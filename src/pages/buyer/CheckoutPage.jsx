...
import { ShieldCheck, CreditCard, CheckCircle2, ArrowLeft, ArrowRight, QrCode, Building2, Truck, Upload, Tag, Copy, Check } from 'lucide-react';
...
export const CheckoutPage = () => {
    ...
    const [uploadedVoucherName, setUploadedVoucherName] = useState(null);
    ...
    return (<div className="min-h-screen bg-heritage-ivory py-8 px-4 sm:px-6 lg:px-8">
      <div className="max-w-5xl mx-auto space-y-8">
        {/* Navigation */}
        <div>
          <Link to="/buyer/cart" className="inline-flex items-center text-xs font-bold text-heritage-brown hover:text-heritage-terracotta transition">
        ...
        <div className="bg-white rounded-3xl p-8 sm:p-12 border-2 border-heritage-gold shadow-3d-lg text-center space-y-6 max-w-2xl mx-auto">
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center gap-3 pt-2">
              <Link to="/buyer/orders" className="w-full sm:w-auto px-6 py-3 rounded-2xl bg-heritage-terracotta hover:bg-heritage-terracotta-dark text-white font-bold text-xs shadow-md transition">
                Track Live Order Delivery
              </Link>
              <Link to="/marketplace" className="w-full sm:w-auto px-6 py-3 rounded-2xl bg-heritage-sand hover:bg-heritage-sand/80 text-heritage-brown font-bold text-xs transition">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
              ...
              <form onSubmit={handleDemoPayment} className="space-y-4">
                <div className="space-y-3">
                  ...
                  <div>
                    ...
                    <input type="text" required value={shippingAddress.street} onChange={(e) => setShippingAddress({ ...shippingAddress, street: e.target.value })} className="w-full p-2.5 rounded-xl border border-heritage-sand text-xs font-medium focus:border-heritage-terracotta outline-none"/>
                  </div>
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    <div>
                      ...
                      <input type="text" required value={shippingAddress.city} onChange={(e) => setShippingAddress({ ...shippingAddress, city: e.target.value })} className="w-full p-2.5 rounded-xl border border-heritage-sand text-xs font-medium focus:border-heritage-terracotta outline-none"/>
                    </div>
                    <div>
                      <label className="block text-xs font-semibold text-heritage-brown mb-1">
                        State
                      </label>
                      <input type="text" required value={shippingAddress.state} onChange={(e) => setShippingAddress({ ...shippingAddress, state: e.target.value })} className="w-full p-2.5 rounded-xl border border-heritage-sand text-xs font-medium focus:border-heritage-terracotta outline-none"/>
                    </div>
                    <div>
                      <label className="block text-xs font-semibold text-heritage-brown mb-1">
                        Pincode
                      </label>
                      <input type="text" required value={shippingAddress.pincode} onChange={(e) => setShippingAddress({ ...shippingAddress, pincode: e.target.value })} className="w-full p-2.5 rounded-xl border border-heritage-sand text-xs font-medium focus:border-heritage-terracotta outline-none"/>
                    </div>
                  </div>
                </div>
                ...
                <div className="pt-4 border-t border-heritage-sand space-y-4">
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                    <button type="button" onClick={() => setPaymentMode('upi')} className={`p-3 rounded-2xl border text-left transition flex flex-col items-start justify-between space-y-2 ${paymentMode === 'upi'
                ...
                : 'border-heritage-sand hover:bg-heritage-ivory/50 bg-white'}`}>
                    ...
                    <button type="button" onClick={() => setPaymentMode('bank_transfer')} className={`p-3 rounded-2xl border text-left transition flex flex-col items-start justify-between space-y-2 ${paymentMode === 'bank_transfer'
                ? 'border-heritage-terracotta bg-heritage-sand/40 shadow-xs ring-1 ring-heritage-terracotta'
                : 'border-heritage-sand hover:bg-heritage-ivory/50 bg-white'}`}>
                    ...
                    <button type="button" onClick={() => setPaymentMode('card')} className={`p-3 rounded-2xl border text-left transition flex flex-col items-start justify-between space-y-2 ${paymentMode === 'card'
                ? 'border-heritage-terracotta bg-heritage-sand/40 shadow-xs ring-1 ring-heritage-terracotta'
                : 'border-heritage-sand hover:bg-heritage-ivory/50 bg-white'}`}>
                    ...
                    <button type="button" onClick={() => setPaymentMode('cod')} className={`p-3 rounded-2xl border text-left transition flex flex-col items-start justify-between space-y-2 ${paymentMode === 'cod'
                ? 'border-heritage-terracotta bg-heritage-sand/40 shadow-xs ring-1 ring-heritage-terracotta'
                : 'border-heritage-sand hover:bg-heritage-ivory/50 bg-white'}`}>
                  ...
                  {paymentMode === 'upi' && (<div className="p-4 rounded-2xl bg-gradient-to-r from-emerald-50/70 to-heritage-sand/30 border border-emerald-200/80 space-y-3 animate-fade-in">
                      <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
                        <div className="space-y-1.5 text-center sm:text-left">
                          ...
                          <div className="inline-flex items-center space-x-1.5 bg-white px-2.5 py-1 rounded-lg border border-heritage-sand text-xs font-mono font-bold text-heritage-charcoal">
                            ...
                            <button type="button" onClick={() => copyToClipboard(sellerBank.upi_id, 'upi')} className="text-heritage-terracotta hover:underline ml-1" title="Copy UPI ID">
                  ...
                  {paymentMode === 'bank_transfer' && (<div className="p-4 rounded-2xl bg-blue-50/60 border border-blue-200 space-y-3 animate-fade-in">
                      <div className="flex items-center justify-between">
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs">
                        ...
                        <div className="p-2.5 bg-white rounded-xl border border-blue-100 flex items-center justify-between">
                          ...
                          <button type="button" onClick={() => copyToClipboard(sellerBank.account_number, 'acc')} className="text-blue-600 p-1 hover:bg-blue-50 rounded">
                        ...
                        <div className="p-2.5 bg-white rounded-xl border border-blue-100 flex items-center justify-between">
                          ...
                          <button type="button" onClick={() => copyToClipboard(sellerBank.ifsc_code, 'ifsc')} className="text-blue-600 p-1 hover:bg-blue-50 rounded">
                      ...
                      <div>
                        ...
                        <input type="text" value={utrNumber} onChange={(e) => setUtrNumber(e.target.value)} placeholder="e.g. 423984920193" className="w-full p-2.5 rounded-xl border border-blue-200 text-xs font-mono font-bold bg-white focus:border-heritage-terracotta outline-none"/>
                      </div>
                    </div>)}

                  {paymentMode === 'card' && (<div className="p-4 rounded-2xl bg-purple-50/60 border border-purple-200 space-y-3 animate-fade-in">
                      <div className="flex items-center justify-between">
                    ...
                    setCardCvv('742');
                      ...
                      <div className="space-y-2 text-xs">
                        <div>
                          ...
                          <input type="text" value={cardNumber} onChange={(e) => setCardNumber(e.target.value)} className="w-full p-2 rounded-xl border border-purple-200 bg-white font-mono font-bold text-xs"/>
                        </div>
                        <div className="grid grid-cols-2 gap-2">
                          <div>
                            <label className="block text-[11px] font-bold text-heritage-brown mb-0.5">Expiry (MM/YY)</label>
                            <input type="text" value={cardExpiry} onChange={(e) => setCardExpiry(e.target.value)} className="w-full p-2 rounded-xl border border-purple-200 bg-white font-mono font-bold text-xs"/>
                          </div>
                          <div>
                            <label className="block text-[11px] font-bold text-heritage-brown mb-0.5">CVV</label>
                            <input type="password" maxLength={4} value={cardCvv} onChange={(e) => setCardCvv(e.target.value)} className="w-full p-2 rounded-xl border border-purple-200 bg-white font-mono font-bold text-xs"/>
                          </div>
                        </div>
                      </div>
                    </div>)}

                  {paymentMode === 'cod' && (<div className="p-4 rounded-2xl bg-amber-50/70 border border-amber-200 text-xs text-amber-900 space-y-1.5 animate-fade-in">
                ...
                <button type="submit" disabled={isProcessing} className="w-full mt-4 py-4 rounded-2xl bg-heritage-terracotta hover:bg-heritage-terracotta-dark text-white font-black text-sm shadow-3d transition flex items-center justify-center space-x-2">
                  {isProcessing ? (<span>Processing Payment to Artisan...</span>) : (<>
            ...
            <div className="lg:col-span-5 bg-white rounded-3xl p-6 border border-heritage-sand shadow-3d space-y-4">
              ...
              <div className="space-y-3 max-h-64 overflow-y-auto pr-1">
                {cart.map((item) => (<div key={item.product.id} className="flex items-center space-x-3 text-xs">
                    <img src={item.product.images[0]} alt={item.product.name} className="w-12 h-12 rounded-xl object-cover border border-heritage-sand"/>
              ...
              <div className="p-4 rounded-2xl bg-gradient-to-r from-heritage-sand/50 via-heritage-ivory to-heritage-sand/40 border border-heritage-terracotta/20 space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  {appliedCoupon && (<button type="button" onClick={handleRemoveCoupon} className="text-[10px] text-red-600 font-bold hover:underline">
                      Remove
                    </button>)}
                </div>

                {!appliedCoupon ? (<div className="space-y-2">
                    <div className="flex items-center space-x-2">
                      <input type="text" value={couponInput} onChange={(e) => setCouponInput(e.target.value.toUpperCase())} placeholder="Enter coupon code" className="flex-1 p-2 rounded-xl border border-heritage-sand bg-white text-xs font-mono uppercase font-bold focus:border-heritage-terracotta outline-none"/>
                      <button type="button" onClick={() => handleApplyCode(couponInput)} className="px-4 py-2 bg-heritage-terracotta hover:bg-heritage-terracotta-dark text-white rounded-xl text-xs font-bold transition shadow-xs">
                        Apply
                      </button>
                    </div>
                    ...
                    <div className="pt-1">
                      <button type="button" onClick={() => voucherInputRef.current?.click()} className="w-full py-2 px-3 border border-dashed border-heritage-terracotta/60 hover:bg-white/80 rounded-xl text-[11px] font-bold text-heritage-terracotta flex items-center justify-center space-x-1.5 transition">
                        <Upload className="w-3.5 h-3.5"/>
                        <span>Upload Voucher / Gift Certificate</span>
                      </button>
                      <input type="file" ref={voucherInputRef} accept="image/*,.pdf" className="hidden" onChange={handleVoucherFileUpload}/>
                    </div>
                    ...
                    <div className="pt-1">
                      ...
                      <div className="flex flex-wrap gap-1.5">
                        <button type="button" onClick={() => handleApplyCode('SIH2026')} className="text-[10px] bg-white border border-heritage-sand hover:border-heritage-terracotta px-2 py-0.5 rounded-lg text-heritage-brown font-mono font-bold">
                          SIH2026 (20% OFF)
                        </button>
                        <button type="button" onClick={() => handleApplyCode('HERITAGE500')} className="text-[10px] bg-white border border-heritage-sand hover:border-heritage-terracotta px-2 py-0.5 rounded-lg text-heritage-brown font-mono font-bold">
                          HERITAGE500 (₹500 OFF)
                        </button>
                        <button type="button" onClick={() => handleApplyCode('ARTISAN15')} className="text-[10px] bg-white border border-heritage-sand hover:border-heritage-terracotta px-2 py-0.5 rounded-lg text-heritage-brown font-mono font-bold">
                          ARTISAN15 (15% OFF)
                        </button>
                      </div>
                    </div>
                  </div>) : (<div className="p-3 bg-emerald-50 rounded-xl border border-emerald-200 text-xs text-emerald-800 space-y-1">
                    <div className="flex items-center justify-between">
                ...
                {couponMessage && (<p className={`text-[10px] font-bold ${couponMessage.isError ? 'text-red-600' : 'text-emerald-700'}`}>
                    {couponMessage.text}
                  </p>)}
              </div>
              ...
              <div className="pt-3 border-t border-heritage-sand text-xs space-y-1.5">
                ...
                {discountAmount > 0 && (<div className="flex justify-between text-emerald-700 font-bold">
