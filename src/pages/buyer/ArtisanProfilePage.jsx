...
export const ArtisanProfilePage = () => {
    ...
    const artisan = DEMO_SELLERS.find((s) => s.username === username || s.id === username) || (currentUser?.role === 'seller' ? currentUser : DEMO_SELLERS[0]);
    const [activeTab, setActiveTab] = useState('products');
    const [inquiryModalOpen, setInquiryModalOpen] = useState(false);
    const [inquiryText, setInquiryText] = useState('');
    const [inquirySent, setInquirySent] = useState(false);
    const artisanProducts = products.filter((p) => p.seller_id === artisan.id || p.seller.username === artisan.username);
    const artisanReviews = reviews.filter((r) => r.seller_id === artisan.id || r.product_name.toLowerCase().includes('krishna'));
