...
import { otpService, OtpDispatchEvent } from '../../services/otpService';

export const OtpNotificationBanner: React.FC = () => {
  const [activeNotification, setActiveNotification] = useState<OtpDispatchEvent | null>(null);
  ...
  const handleAutoFill = () => {
    if (activeNotification) {
      window.dispatchEvent(
        new CustomEvent('karigarsetu-autofill-otp', {
  ...
  return (
    <div className="fixed top-4 left-1/2 -translate-x-1/2 z-[9999] w-[92%] sm:w-[480px] max-w-full animate-bounce-short">
        ...
        <div className="flex items-center justify-between pb-2 border-b border-white/10 mb-2.5">
          <div className="flex items-center space-x-2">
            <div className={`w-7 h-7 rounded-lg flex items-center justify-center ${
              isMobile ? 'bg-emerald-600 text-white' : 'bg-amber-600 text-white'
            }`}>
        ...
        <div className="space-y-2">
          ...
          <div className="flex items-center space-x-2 pt-1">
            <button
              ...
              className="flex-1 py-1.5 px-3 rounded-lg bg-heritage-terracotta hover:bg-heritage-terracotta-dark text-white font-bold text-xs flex items-center justify-center space-x-1.5 shadow-sm transition"
            >
            ...
            <button
              type="button"
              onClick={handleCopy}
              className="py-1.5 px-3 rounded-lg bg-white/15 hover:bg-white/25 text-white font-bold text-xs flex items-center justify-center space-x-1 transition"
            >
              {copied ? (
                <>
