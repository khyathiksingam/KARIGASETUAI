...
import { Camera, X, RefreshCw, SwitchCamera, AlertCircle, Check, Sparkles } from 'lucide-react';
      ...
      <div className="bg-heritage-charcoal rounded-3xl overflow-hidden border border-heritage-gold/40 shadow-3d-lg max-w-xl w-full flex flex-col relative text-white">
          ...
          {errorMsg ? (
            <div className="p-6 text-center space-y-3 max-w-sm">
              ...
              <button
                ...
                className="px-4 py-2 bg-heritage-terracotta text-white rounded-xl text-xs font-bold shadow-md hover:bg-heritage-terracotta-dark transition"
              >
        ...
        <div className="p-4 bg-heritage-brown/95 border-t border-heritage-gold/20 flex items-center justify-between">
          <button
            ...
            className="p-3 rounded-2xl bg-white/10 hover:bg-white/20 text-heritage-gold-light transition flex items-center space-x-1.5 text-xs font-bold disabled:opacity-50"
          >
          ...
          <button
            type="button"
            disabled={Boolean(errorMsg) || isInitializing}
            onClick={handleCaptureFrame}
            className="w-16 h-16 rounded-full bg-gradient-to-r from-heritage-terracotta to-heritage-gold p-1 shadow-glow-terracotta hover:scale-105 active:scale-95 transition-all flex items-center justify-center disabled:opacity-50"
            title="Take Photo"
          >
          ...
          <button
            type="button"
            onClick={() => fileFallbackRef.current?.click()}
            className="px-3 py-2 rounded-xl bg-white/10 hover:bg-white/20 text-xs font-bold text-heritage-sand transition"
          >
            Mobile Camera
          </button>
        </div>
