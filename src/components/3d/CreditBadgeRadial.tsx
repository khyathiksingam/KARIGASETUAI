...
import { Award, Sparkles, ShieldCheck, ChevronRight, TrendingUp } from 'lucide-react';
import { ArtisanBadge } from '../../types';

interface CreditBadgeRadialProps {
  credits: number;
  badge: ArtisanBadge;
  size?: 'sm' | 'md' | 'lg';
  showDetails?: boolean;
...
export const CreditBadgeRadial: React.FC<CreditBadgeRadialProps> = ({
  credits,
  badge,
  size = 'md',
  showDetails = true,
  ...
  const progressPercent = Math.min(
    100,
    Math.max(5, Math.round(((credits - prevThreshold) / (nextThreshold - prevThreshold)) * 100))
  );
      ...
      {showDetails && (
        <div className="mt-2.5 w-full max-w-[220px]">
          ...
          <div className="w-full bg-heritage-sand rounded-full h-1.5 overflow-hidden border border-heritage-terracotta/20">
            <div
              className="bg-gradient-to-r from-heritage-gold to-heritage-terracotta h-full rounded-full transition-all duration-700"
